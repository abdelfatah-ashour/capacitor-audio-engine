...
public class PermissionManagerService {
    ...
    public JSObject checkPermissions() {
        Log.d(TAG, "Checking detailed permissions");

        // Check microphone permission
        JSObject microphoneResult = checkSinglePermissionStatus(PermissionType.MICROPHONE);

        // Check notification permission
        JSObject notificationResult = checkSinglePermissionStatus(PermissionType.NOTIFICATIONS);

        // Determine overall granted status
        boolean microphoneGranted = "granted".equals(microphoneResult.getString("status"));
        boolean notificationGranted = "granted".equals(notificationResult.getString("status")) ||
                                     "unsupported".equals(notificationResult.getString("status"));
        boolean overallGranted = microphoneGranted && notificationGranted;

        JSObject result = new JSObject();
        result.put("granted", overallGranted);
        result.put("microphone", microphoneResult);
        result.put("notifications", notificationResult);

        // Add platform info
        JSObject platformInfo = new JSObject();
        platformInfo.put("androidApiLevel", Build.VERSION.SDK_INT);
        result.put("platformInfo", platformInfo);

        Log.d(TAG, "Detailed permission check completed - Overall granted: " + overallGranted);
        return result;
    }
    ...
    public JSObject checkPermissionMicrophone() {
        ...
        return checkSinglePermissionStatus(PermissionType.MICROPHONE);
    }
    ...
    public JSObject checkPermissionNotifications() {
        ...
        return checkSinglePermissionStatus(PermissionType.NOTIFICATIONS);
    }
     ...
     */
    public void requestDetailedPermissions(PluginCall call, JSObject options) {
