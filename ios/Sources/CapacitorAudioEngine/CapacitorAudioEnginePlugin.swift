...
public class CapacitorAudioEnginePlugin: CAPPlugin, CAPBridgedPlugin, RecordingManagerDelegate, PlaybackManagerDelegate, WaveLevelEventCallback {
    ...
    public let pluginMethods: [CAPPluginMethod] = [
        ...
        CAPPluginMethod(name: "requestDetailedPermissions", returnType: CAPPluginReturnPromise),
    ...
    @objc func requestDetailedPermissions(_ call: CAPPluginCall) {
        Task {
            let options = call.getObject("options")
            let result = await permissionService.requestDetailedPermissions(options: options)
            await MainActor.run {
                call.resolve(result)
            }
        }
